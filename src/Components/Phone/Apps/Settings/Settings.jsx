...
import "./settings.css";
import {
    UIXPhoneSetDarkBackground, UIXPhoneSetPlaneMode, UIXPhoneSetWiFiMode, UIXPhoneSetMobileDataMode, UIXPhoneSetDarkMode,
    UIXPhoneSetDisturbeMode, UIXPhoneSetPath, UIXPhoneSetAnimationDuration, UIXPhoneSetAnitmation
} from "../../../../redux/actions/phoneActions";

export class Settings extends Component {

    UIXPhoneSetPlaneMode = () => this.props.UIXPhoneSetPlaneMode(!this.props.settings.planeMode)
    UIXPhoneSetWiFiMode = () => this.props.UIXPhoneSetWiFiMode(!this.props.settings.wifiEnabled)
    UIXPhoneSetMobileDataMode = () => this.props.UIXPhoneSetMobileDataMode(!this.props.settings.mobileDataEnabled)
    UIXPhoneSetDarkMode = () => this.props.UIXPhoneSetDarkMode(!this.props.settings.darkMode)
    UIXPhoneSetDisturbeMode = () => this.props.UIXPhoneSetDisturbeMode(!this.props.settings.disturbeMode)

    render() {
        const { settings, UIXPhoneSetDarkBackground, t } = this.props
        const { darkMode, planeMode, wifiEnabled, mobileDataEnabled, disturbeMode } = settings
            ...
            s1: [
                {
                    ...
                    action:
                        <label className="form-switch">
                            <input readOnly checked={planeMode} type="checkbox" />
                ...
                },
                {
                    ...
                    action:
                        <label className="form-switch">
                            <input readOnly checked={wifiEnabled} type="checkbox" />
                            <i className={darkMode ? "form-switch-i-dark" : "form-switch-i-light"} />
                        </label>
                },
                {
                    ...
                    action:
                        <label className="form-switch">
                            <input readOnly checked={mobileDataEnabled} type="checkbox" />
                            <i className={darkMode ? "form-switch-i-dark" : "form-switch-i-light"} />
                        </label>
                },
                {
                    ...
                    action:
                        <label className="form-switch">
                            <input readOnly checked={darkMode} type="checkbox" />
                            <i className={darkMode ? "form-switch-i-dark" : "form-switch-i-light"} />
                        </label>
                },
                {
                    label: t("apps.phone.apps.settings.disturbe-mode-label"),
                    iconStyle: { background: "rgb(94, 92, 230)" },
                    icon: <i className="fas fa-moon" />,
                    func: "UIXPhoneSetDisturbeMode",
                    action:
                        <label className="form-switch">
                            <input readOnly checked={disturbeMode} type="checkbox" />
                            <i className={darkMode ? "form-switch-i-dark" : "form-switch-i-light"} />
                        </label>
                }
            ],
            s2: [
                {
                    ...
                    action:
                        <div className="settings-option-arrow" >
                            <i className="fas fa-chevron-right" style={{ color: color_t }} />
                        </div >
                },
                {
            ...
            <div className="app">
                <div className="settings" style={{ background: color_p }}>
                    ...
                    <div className="settings-top" style={{ background: color_s }}>
                        <div className="settings-label" style={{ color: color_t }}>{t("apps.phone.apps.settings.label")}</div>
                    </div>

                    <div className="settings-middle">
                        <div className="settings-middle-s1">
                            {config.s1.map(({ label, icon, iconStyle, action, func }, i) => (
                                <div key={i} className="settings-option" data-function={func} data-class="settings-option-selected" data-pos={[i, 0]} style={{ background: color_s }}>
                                    <div className="settings-option-icon">
                                        <div style={iconStyle}>{icon}</div>
                                    </div>
                                    <div className="settings-option-label" style={{ color: color_t }}>{label}</div>
                        ...
                        <div className="settings-middle-s2">
                            {config.s2.map(({ label, icon, iconStyle, action, func }, i) => (
                                <div key={i} className="settings-option" data-function={func} data-class="settings-option-selected" data-pos={[i + 5, 0]} style={{ background: color_s }}>
                                    <div className="settings-option-icon">
                                        <div style={iconStyle}>{icon}</div>
                                    </div>
                                    <div className="settings-option-label" style={{ color: color_t }}>{label}</div>
    ...
    enter = ({ dataset }) => { this[dataset.function]() }
    exited = () => { this.props.UIXPhoneSetDarkBackground(true) }
    ...
    entered = () => {
        ...
        const elements = [
            ...Array.from(document.getElementsByClassName("settings-middle-s1")[0].childNodes),
            ...Array.from(document.getElementsByClassName("settings-middle-s2")[0].childNodes)
        ]
