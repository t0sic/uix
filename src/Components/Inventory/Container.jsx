        ...
        $(".inventory-container-slot").droppable({
            ...
            drop: function (event, ui) {
                if (this.children.length) {
                    ...
                    const slot2 = { slot: $(ui.draggable).data("item").item.slot, action: $(ui.draggable).data("item").action }
                    const action1 = $(ui.draggable).data("item").action
                    const action2 = JSON.parse(this.children[0].dataset.item).action
                    container.props.onDrop({ item: item2, slot: slot2, action: action2 })
                    container.props.onDrop({ item: item1, slot: slot1, action: action1 })
                } else {
    ...
    slots = () => {
        ...
        for (let i = 0; i < slots; i++) {
            ...
            for (let j = 0; j < items.length; j++) {
                if (items[j].slot === i) {
                    item = <Item action={action} item={items[j]} />
                }
            }
            output.push(<div className="inventory-container-slot" key={i} data-slot={JSON.stringify({ slot: i, action })}>{item}</div>)
        }
